...
module OrderTaking.DomainTypes.ProductCodeSpec
  ( spec
  ) where
...
spec = do
  describe "ProductCode" $ do
    ...
    it "should fail to create product code hoge" $ do
      ProductCode.create "hoge"
        `shouldBe` Left "product code hoge is invalid. must start with G or W"

    it "should fail to create product code W123" $ do
      ProductCode.create "W123"
        `shouldBe` Left "widget code W123 is invalid. must be Wxxxx"

    it "should fail to create product code G12" $ do
      ProductCode.create "G12"
        `shouldBe` Left "gizmo code G12 is invalid. must be Gxxx"
