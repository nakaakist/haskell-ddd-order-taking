...
module OrderTaking.Types.CustomerInfoSpec
  ( spec
  ) where
...
spec = do
  ...
  describe "PersonalName" $ do
    it "should success to create name with length up to 50" $ do
      let params = PersonalName.Params { PersonalName.firstName = "nariyuki"
    ...
    it "should fail to create name with length more than 50" $ do
      let params = PersonalName.Params { PersonalName.firstName = longName
                                       , PersonalName.lastName  = "saito"
                                       }
